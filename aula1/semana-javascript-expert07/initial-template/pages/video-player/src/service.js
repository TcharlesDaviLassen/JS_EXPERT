...
const { shouldRun } = prepareRunChecker({ timerDelay: 900 })
...
export default class Service {
    ...
    #getEAR(upper, lower) {
        ...
        return ((getEucledianDistance(upper[5][0], upper[5][1], lower[4][0], lower[4][1])
    ...
    async handBlinked(video) {
        // console.log('handBlinked')
        const predections = await this.#estimateFaces(video)

        if (!predections.length) { return false }

        for (const prediction of predections) {
